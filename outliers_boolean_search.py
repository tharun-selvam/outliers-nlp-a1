...
def preprocess_query_term(term: str, stemmer: PorterStemmer):
    """
    Applies the same important preprocessing steps used
    ...
    The assignment specifies that query terms must
    be normalized and stemmed.
...
def intersect_postings(postings1, postings2):
    ...
    Both postings lists are already sorted.

    We therefore use the standard two-pointer
...
def main(argv=None):
    ...
    try:
        ...
        # ---------------------------------------------
        # Query can either be supplied through:
        #
        # --query
        #
