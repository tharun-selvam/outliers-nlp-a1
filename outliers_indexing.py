...
def read_processed_file(input_path:Path):
    """
    Input: Preprocessed collection.
    Output: Dictionary mapping: token -> set of document IDs
    """
...
def write_index_file(index, max_docid, output_path: Path):
    ...
    Sort each postings list numerically.
    Write the vocabulary size (number of tokens) and the maximum docid as the first line.
    Write each token and its postings list.
    """
